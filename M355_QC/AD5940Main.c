...
v2.2.0 removed chip insertion related requirements for PCB V2.6 and above.
v1.0.0 initial version for QC purpose only.
Removed all the code related to running measurement swv.
*/
#define FMWARE_VERSION "1.0.0"
...
Set bits to 0 turn LED ON, set bit to 1 turn LED off. 
*/
...
This is controling the onBoard led and reader LED.
status is the Board QC result, 0 means no error,
otherwise is a bitMask of incorrect channels. 
*/
#define INTERVAL 99999  // this equals roughly 1s flash
void showStatusLED(uint8_t* status) {	
	static uint8_t ledState = 0x00; // default state is on
	static uint8_t onBoardLEDState = 0x1; // default state is off.
	static uint32_t count = 0;	
	if (*status != 0) { 
		// QC failed then blink on board LED
		if (count % INTERVAL == 0) {			
			onBoardLEDState ^= 0x1;
		}
	} 
	// always blink white LED for the reader LED.
	if (count % INTERVAL == 0) {
			ledState ^= 0x19;
	}
	turnLED(ledState);
...
{	
	...
	{			
		...
		printf("{\"e\":\"This firmware only support QC routine.\"}*");
