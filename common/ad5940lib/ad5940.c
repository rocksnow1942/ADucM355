...
void AD5940_ClksCalculate(ClksCalInfo_Type *pFilterInfo, uint32_t *pClocks) {
  uint32_t temp = 0;
  const uint32_t sinc3osr_table[] = {5,4,2,0};																						// ADCFILTERCON.SINC3OSR[13:12]	
  const uint32_t sinc2osr_table[] = {22,44,89,178,267,533,640,667,800,889,1067,1333,0};   // ADCFILTERCON.SINC2OSR[11:8]
...
float AD5940_ADCCode2Volt(uint32_t code, uint32_t ADCPga, float VRef1p82)
{
  ...
  float fVolt = 0.0;
  float tmp = 0;
  tmp = (int32_t)code - 32768;   // 2^15=32768=0x8000
  switch(ADCPga)
  {
  case ADCPGA_1:
    break;
...
{
  while(uiReadCount--)
    *pBuffer++ = *(volatile uint32_t *)(0x400c206C);  // DATAFIFORD : Data FIFO read register
}
#else
/**
 * @defgroup SPI_Block
...
void AD5940_Initialize(void)
{
  ...
  if(i == 0x5501)
    ...
    bIsS2silicon = bTRUE;  // R-ADuCM355 Fermi, 7th July
  else if(i == 0x5500)  
    bIsS2silicon = bFALSE;
#ifdef ADI_DEBUG
  else
...
void AD5940_AFECtrlS(uint32_t AfeCtrlSet, BoolFlag State) {
  ...
  if (State == bTRUE) {
    ...
    if(AfeCtrlSet & AFECTRL_HPREFPWR) {  		// AFECON.[5] : High power reference on-off control
        tempreg |= BITM_AFE_AFECON_HPREFDIS;
        AfeCtrlSet &= ~AFECTRL_HPREFPWR;
    }
    if(AfeCtrlSet & AFECTRL_ALDOLIMIT) {		// AFECON.[19] : ALDO current limit on-off control 
      tempreg |= BITM_AFE_AFECON_ALDOILIMITEN;
      AfeCtrlSet &= ~AFECTRL_ALDOLIMIT;
    }
    tempreg &= ~AfeCtrlSet;  
    // Only AFECON.[5, 19] is "1" which means disable, and other bits are "0', which means disable.
    // So AFECON bits makes AFE disabled.
  }
...
void AD5940_FIFOCfg(FIFOCfg_Type *pFifoCfg)
{
  ...
  AD5940_WriteReg(REG_AFE_CMDDATACON, tempreg);   // tempreg=0x450, Data FIFO is FIFO mode, size is 2[kB]
  ...
  AD5940_WriteReg(REG_AFE_FIFOCON, tempreg);  // tempreg=0x800, DATAFIFOEN=1, Data FIFO source is ADC data from sinc3 filter.
}
...
void AD5940_SEQCfg(SEQCfg_Type *pSeqCfg)
{
  ...
  AD5940_WriteReg(REG_AFE_CMDDATACON, tempreg);  // tempreg=0x44A, CMD memory 4[kB], 
  ...
  AD5940_WriteReg(REG_AFE_SEQCON, tempreg);  // tempreg=0,
  AD5940_WriteReg(REG_AFE_FIFOCON, fifocon);  // restore FIFO configuration, fifocon=0x800
}
/**
...
void AD5940_CLKCfg(CLKCfg_Type *pClkCfg)
{
  ...
  if(pClkCfg->HFXTALEn == bTRUE) {
	...
	// AFE high frequency oscillator is a 16 MHz or 32 MHz internal oscillator.
  if(pClkCfg->HFOSCEn == bTRUE) {
    ...
    if(pClkCfg->HfOSC32MHzMode  == bTRUE)
      AD5940_HFOSC32MHzCtrl(bTRUE);		// Now, AFE high frequency oscillator is 32 MHz internal oscillator.
    else
      AD5940_HFOSC32MHzCtrl(bFALSE);  // Now, AFE high frequency oscillator is 16 MHz internal oscillator.
  }
	// --> Now, we configured the AFE high frequency either 16[MHz] or 32[MHz].
  ...
  if(pClkCfg->LFOSCEn == bTRUE) {
	...
	// --> Now, we configured the AFE low frequency 32[kHz].

  /* Switch clocks */
  // step1. Set clock divider  : AFE High frequency will be used without any divider.
  ...
  AD5940_WriteReg(REG_AFECON_CLKCON0, tempreg); // tempreg=0x41, which SysClkDiv=1, ADCClkDiv=1
  AD5940_Delay10us(10);
  // Step2. set clock source : ADC clock source and AFE system clock will be AFE internal high frequency
  tempreg = pClkCfg->SysClkSrc;
  tempreg |= pClkCfg->ADCCLkSrc << BITP_AFECON_CLKSEL_ADCCLKSEL;
  AD5940_WriteReg(REG_AFECON_CLKSEL, tempreg);  // tempreg=0, which SysClkSrc=0, ADCCLkSrc=0

  /* Disable clocks */
  if(pClkCfg->HFXTALEn == bFALSE)
    reg_osccon &= ~BITM_ALLON_OSCCON_HFXTALEN;  // reg_osccon=0x303
...
AD5940Err AD5940_LPRtiaCal(LPRTIACal_Type *pCalCfg, void *pResult) {
  ...
  /* Calculate the excitation voltage we should use based on RCAL/Rtia */
  RtiaVal = LpRtiaTable[pCalCfg->LpTiaRtia];  // RtiaVal=110 due to pCalCfg->LpTiaRtia=1
  /*
   * DAC output voltage calculation
   * Note: RCAL value should be similar to RTIA so the accuracy is best.
 ...
 /** @cond */
  ...
  #define __MAXVOLT_AMP_CODE  (MAXVOLT_P2P*2047L/2200)   // 1400*2047/2200=1302
 /** @endcond */
  ExcitVolt = 2000*0.8*pCalCfg->fRcal/RtiaVal;  // 2000*0.8*200/110=2909
  WgAmpWord = ((uint32_t)(ExcitVolt/2200*2047*2)+1)>>1; /* WgAmpWord=2707, Assign value with rounding (0.5 LSB error) */
  ...
  */
  {
    ...
    ExcitVolt = WgAmpWord*2000.0f/2047; /* WgAmpWord=1302, ExcitVolt=1272, 2000mVpp -->ExcitVolt in Peak to Peak unit */
    RtiaVolt = ExcitVolt/(pCalCfg->fRcal + 100)*RtiaVal;  // RtiaVolt=466
    RcalVolt = RtiaVolt/RtiaVal*pCalCfg->fRcal;           // RcalVolt=848
    /* The input range of ADC is 1.5Vp, we calculate how much gain we need */
    temp = 3000.0f/RcalVolt;  // temp=3.53744244
    if(temp >= 9.0f)  ADCPgaGainRcal = ADCPGA_9;
    else if(temp >= 4.0f) ADCPgaGainRcal = ADCPGA_4;
    else if(temp >= 2.0f) ADCPgaGainRcal = ADCPGA_2;  // ADCPGA_2=2, it is selected!
    else if(temp >= 1.5f) ADCPgaGainRcal = ADCPGA_1P5;
    else ADCPgaGainRcal = ADCPGA_1;
    temp = 3000.0f/RtiaVolt;  // temp=6.4317135811
    if(temp >= 9.0f)  ADCPgaGainRtia = ADCPGA_9;
    else if(temp >= 4.0f) ADCPgaGainRtia = ADCPGA_4;  // ADCPGA_4=3, it is selected!
    else if(temp >= 2.0f) ADCPgaGainRtia = ADCPGA_2;
    else if(temp >= 1.5f) ADCPgaGainRtia = ADCPGA_1P5;
    else ADCPgaGainRtia = ADCPGA_1;
    GainRatio = ADCPGAGainTable[ADCPgaGainRtia]/ADCPGAGainTable[ADCPgaGainRcal];  // GainRatio=4/2=2
  }
  ...
  /* Configure reference system */
	...
	dsp_cfg.ADCFilterCfg.ADCAvgNum = ADCAVGNUM_16;  /* ADCAvgNum=3, Don't care because it's disabled */
	dsp_cfg.ADCFilterCfg.ADCRate = bADCClk32MHzMode?ADCRATE_1P6MHZ:ADCRATE_800KHZ;  // ADCRate=ADCRATE_800KHZ(1)
	dsp_cfg.ADCFilterCfg.ADCSinc2Osr = pCalCfg->ADCSinc2Osr;
	dsp_cfg.ADCFilterCfg.ADCSinc3Osr = pCalCfg->ADCSinc3Osr;
	dsp_cfg.ADCFilterCfg.BpNotch = bTRUE;
	dsp_cfg.ADCFilterCfg.BpSinc3 = bFALSE;
  ...
  lp_loop.LpAmpCfg.LpTiaRtia = pCalCfg->LpTiaRtia;  // pCalCfg->LpTiaRtia=1, 200[Ohm]
  ...
  {
		hs_loop.WgCfg.SinCfg.SinAmplitudeWord = WgAmpWord;  // WgAmpWord=1302
		hs_loop.WgCfg.SinCfg.SinFreqWord = AD5940_WGFreqWordCal(pCalCfg->fFreq, pCalCfg->SysClkFreq); // SinFreqWord=17873, when pCalCfg->fFreq=2663, pCalCfg->SysClkFreq=16000000
		hs_loop.WgCfg.SinCfg.SinOffsetWord = 0;
		hs_loop.WgCfg.SinCfg.SinPhaseWord = 0;
    ...
    pADCBaseCfg->ADCMuxN = ADCMUXN_N_NODE;
    pADCBaseCfg->ADCMuxP = ADCMUXP_P_NODE;
    pADCBaseCfg->ADCPga = ADCPgaGainRcal;    // ADCPgaGainRcal=2;
    AD5940_ADCBaseCfgS(pADCBaseCfg);
...
AD5940Err AD5940_LFOSCMeasure(LFOSCMeasure_Type *pCfg, float *pFreq) /* Measure current LFOSC frequency. */
{
  ...
  seq_cfg.SeqBreakEn = bFALSE;				// Reserved
  seq_cfg.SeqIgnoreEn = bFALSE;				// Reserved
