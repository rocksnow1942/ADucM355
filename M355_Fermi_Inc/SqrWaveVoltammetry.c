 ...
 * @note The method to calculate LPDAC output voltage
...
AD5940Err AppSWVCtrl(uint32_t Command, void *pPara) {
  switch (Command) {
    case APPCTRL_START:   // start the measurment. Note: the test needs firstly to call function AppSWVInit() every time before start it.
    {
    ...
    case APPCTRL_STOPNOW:		// Stop the measurment immediately.
    {
      if(AD5940_WakeUp(10) > 10)  /* Wakeup AFE by read register, read 10 times at most */
        return AD5940ERR_WAKEUP;  /* Wakeup Failed */
    ...
    case APPCTRL_STOPSYNC:  // Stop the measuremnt when current measured data is read back.
    {
      AppSWVCfg.StopRequired = bTRUE;
      break;
    }
    case APPCTRL_SHUTDOWN:   // Stop the measurment immediately and put AFE to shut down mode(turn off LP loop and enter hibernate).
    {
...
static AD5940Err AppSWVSeqInitGen(void) {
  ...
  // Fermi 0627
	AD5940_SEQGenInsert(SEQ_WAIT(16*gsecsPretreatment*1000)); /* wait 200000us=200[ms] for reference power up */
...
//    SeqCmdBuff[1] = SEQ_WR(REG_AFE_LPDACDAT1, 0);
  ...
  if(AppSWVCfg.bFirstDACSeq) {
    ...
    if(bIsFinalBlk == bFALSE) { /* Otherwise there is no need to init block1 sequence */
...
static AD5940Err AppADCPgaOffsetCal(void)
{
  ...
  pga_cal.AdcClkFreq = AppSWVCfg.AdcClkFreq;
  pga_cal.SysClkFreq = AppSWVCfg.SysClkFreq;
...
AD5940Err AppSWVInit(uint32_t *pBuffer, uint32_t BufferSize) {
  ...
  // Step 1: configure sequencer. Configure sequencer and stop it
  ...
  AD5940_INTCClrFlag(AFEINTSRC_ALLINT);  // all interrupt flags are cleared
  /* Generate DAC sequence SEQID0 and SEQID1*/
...
AD5940Err AppSWVISR(void *pBuff, uint32_t *pCount) {
  ...
  *pCount = 0;
  ...
  IntFlag = AD5940_INTCGetFlag(AFEINTC_0);
	// Custom#0 Interrupt,	High priority :
